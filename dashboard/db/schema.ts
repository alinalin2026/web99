/* Runtime schema used by /api/setup and lazy master-dashboard migration.
   Keep this string in sync with schema.sql. Every statement is idempotent. */
...
CREATE TABLE IF NOT EXISTS orders (
  ...
  business_name text, trade text, location text, email text, phone text,
  conversation jsonb NOT NULL DEFAULT '[]'::jsonb,
  brief jsonb, analysis jsonb, generated jsonb, generator_notes text,
  slug text UNIQUE, preview_url text, commit_sha text,
  stripe_session_id text, stripe_payment_intent text, paid_at timestamptz,
  retention text CHECK (retention IN ('stayed','left')),
  failure_reason text, approved_by text, approved_at timestamptz,
  sent_at timestamptz, expires_at timestamptz,
  ...
  plan_text text, studio_copy text, studio_data jsonb,
  build_provider text, build_job_id text, qa_report jsonb,
...
CREATE TRIGGER project_assets_touch BEFORE UPDATE ON project_assets FOR EACH ROW EXECUTE FUNCTION touch_updated_at();
`;

export const MASTER_MIGRATION_SQL = SETUP_SQL;