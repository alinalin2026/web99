/* Anthropic wrapper for Sarah, extraction, planning, studio and QA. */
...
export const MODELS = {
  ...
  studio: process.env.STUDIO_MODEL ?? process.env.ANALYST_MODEL ?? "claude-sonnet-4-6",
  qa: process.env.QA_MODEL ?? process.env.ANALYST_MODEL ?? "claude-sonnet-4-6",
} as const;
...
async function complete(
  ...
  try { data = raw ? (JSON.parse(raw) as AnthropicResponse) : {}; }
  catch { throw new Error(`Anthropic returned invalid JSON (HTTP ${response.status}): ${raw.slice(0, 300)}`); }
...
export async function json<T = unknown>(
  ...
  const raw = await complete(jsonSystem, [{ role: "user", content: user }], model, maxTokens, 0.3);
